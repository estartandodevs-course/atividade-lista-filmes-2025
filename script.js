...
function buscarPorTítulo(event) {
  ...
  const filmesFiltrados = filmes.filter((filme) =>
    filme.titulo.toLowerCase().includes(textoBusca)
  );
...
function ordenarPorAno_E_Nota() {
  ...
  botoesOrdenarAno.addEventListener("click", () => {
    const filmesOrdenados = listarParaRenderizar
      .slice()
...
function adicionarFilmeEvento() {
  ...
  listarFilmes.addEventListener("click", (event) => {
    ...
    if (event.target.classList.contains("btn-adicionar")) {
      ...
      const filmeParaAdicionar = filmes.find(
        (filme) => filme.id === parseInt(filmeId)
      );
      if (
        filmeParaAdicionar &&
        !minhaLista.some((filme) => filme.id === filmeParaAdicionar.id)
      ) {
...
function modalDeDetalhes() {
  ...
  containerFilme.addEventListener("click", (event) => {
    ...
    const filmeDetalhes = filmes.find(
      (filme) => filme.id === parseInt(filmeId)
    );
...
function calcularEstatisticas() {
  ...
  const notaMedia =
    totalFilmes === 0
      ? 0
      : (
          minhaLista.reduce((acc, filme) => acc + filme.nota, 0) / totalFilmes
        ).toFixed(2);
